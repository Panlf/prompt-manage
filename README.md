...
[功能特性](#-功能特性) · [快速开始](#-快速开始) · [构建打包](#-构建与打包) · [架构](#-架构) · [文档](#-文档)
...
**Q：开发模式（`bun run dev`）控制台出现 `Failed to read version.json ... ENOENT` 报错？**
这是 electrobun 框架内部更新器在**开发模式**下的已知噪音：项目根目录没有 `../Resources/version.json`，框架内部已捕获该异常并回退，**不影响任何功能**。打包后的应用自带该文件（`Resources/version.json`），不会出现此报错。

**Q：新建/编辑弹窗会不会误触丢失内容？**
不会。业务弹窗不支持点击遮罩关闭（只能「取消」按钮、Esc 或保存），且已填内容作为草稿保留——误关后重新打开自动恢复，仅在保存成功或切换页面时清空。

**Q：导出数据去哪里了？**
点「导出为 JSON」会先弹出**系统文件夹选择器**让你指定位置，成功后 Toast 明确提示导出到哪个文件夹；若运行环境不支持选择器，则回退为浏览器下载（系统「下载」文件夹）。
...
| 文档 | 内容 |
|------|------|
| [docs/PRD.md](docs/PRD.md) | 产品需求文档：竞品调研、热度矩阵、功能验收标准 |
| [docs/DESIGN.md](docs/DESIGN.md) | 设计规范：视觉与交互规范 |
| [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) | 开发文档：架构、数据库变更、RPC 接口、重构清单 |
| [docs/TESTING.md](docs/TESTING.md) | 测试报告：单测明细、迁移验证、构建验证、冒烟清单 |
